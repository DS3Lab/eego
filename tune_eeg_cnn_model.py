...
def main():
    ...
    count = 0
    n_iter = len(config.random_seed_values) * len(config.lstm_dim) * len(config.lstm_layers) * len(config.dense_dim) * len(config.cnn_network) * len(config.dropout) * len(config.batch_size) * len(config.epochs)
                                ...
                                for lr_val in config.lr:
                                    for e_val in config.epochs:
                                        parameter_dict = {"lr": lr_val, "lstm_dim": lstmDim, "lstm_layers": lstmLayers,
                                                        ...
                                                        "epochs": e_val, "random_seed": rand, "cnn_network": cnn_network}
