        ...
        if "rawEEG" in available_objects:
            ...
            for raw_eegs_obj, ets_obj, word_obj, ffd, gd, gpt, trt, sfd, nFix, fixPos in zipped_data:
                ...
                if is_real_word(word_string):
                    ...
                    data_dict["FFD"] = data_container[ffd[0]].value[0, 0] if len(
                        data_container[ffd[0]].value.shape) == 2 else None
                    data_dict["GD"] = data_container[gd[0]].value[0, 0] if len(
                        data_container[gd[0]].value.shape) == 2 else None
                    data_dict["GPT"] = data_container[gpt[0]].value[0, 0] if len(
                        data_container[gpt[0]].value.shape) == 2 else None
                    data_dict["TRT"] = data_container[trt[0]].value[0, 0] if len(
                        data_container[trt[0]].value.shape) == 2 else None
                    data_dict["SFD"] = data_container[sfd[0]].value[0, 0] if len(
                        data_container[sfd[0]].value.shape) == 2 else None
                    data_dict["nFix"] = data_container[nFix[0]].value[0, 0] if len(
                        data_container[nFix[0]].value.shape) == 2 else None
