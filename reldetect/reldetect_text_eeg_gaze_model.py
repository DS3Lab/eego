...
# Machine learning model for relation detection (binary and ternary)
# Jointly learning from text AND cognitive word-level EEG features AND gaze features
...
def classifier(features, labels, eeg, gaze, embedding_type, param_dict, random_seed_value, threshold):
    ...
    for train_index, test_index in kf.split(X_data_text):
        ...
        if config.model is 'lstm':
            eeg_model_model = create_lstm_cognitive_model(param_dict, (X_train_eeg.shape[1], X_train_eeg.shape[2]), 'eeg_input_tensor')
        elif config.model is 'cnn':
            eeg_model_model = create_inception_cognitive_model(param_dict, (X_train_eeg.shape[1], X_train_eeg.shape[2]), 'eeg_input_tensor')
