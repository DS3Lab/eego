...
def create_lstm_word_model(param_dict, embedding_type, X_train_shape, num_words, text_feats, y_train_shape):  # X_train_shape = X_train_text.shape[1]
...
def classifier(features, labels, embedding_type, param_dict, random_seed_value):
    ...
    y, X = ml_helpers.drop_samples(y, X)
    ...
    for train_index, test_index in kf.split(X_data_text):
        ...
        model = create_lstm_word_model(param_dict, embedding_type, X_train_text.shape[1], num_words,
                                                  text_feats)
        ...
        label_names = ["JobTitle", "NoRel"]

        p, r, f, support = sklearn.metrics.precision_recall_fscore_support(rounded_labels, rounded_predictions,
                                                                           average='macro')
        print(p, r, f)
        print(sklearn.metrics.classification_report(rounded_labels, rounded_predictions, target_names=label_names))
        print(sklearn.metrics.classification_report(rounded_labels, rounded_predictions, target_names=label_names, output_dict=True))
