...
def lstm_classifier(labels, eeg, gaze, embedding_type, param_dict, random_seed_value, threshold):
    ...
    for train_index, test_index in kf.split(X_data_gaze):
        ...
        # the first branch operates on the first input (gaze data)
        ...
        combined = concatenate([gaze_model_model.output, cognitive_model_model.output])
        ...
        model = Model(inputs=[gaze_model_model.input, cognitive_model_model.input], outputs=combi_model)
        ...
        # train model
        history = model.fit([X_train_gaze, X_train_eeg], y_train,
        ...
        scores = model.evaluate([X_test_gaze, X_test_eeg], y_test,
                                verbose=0)
        predictions = model.predict([X_test_gaze, X_test_eeg])
