...
def lstm_classifier(features, labels, eeg, gaze, embedding_type, param_dict, random_seed_value):
    ...
    for train_index, test_index in kf.split(X_data_text):
        ...
        scores = saved_model.evaluate([X_test_text, X_test_eeg, X_test_gaze] if embedding_type is not 'bert' else [X_test_text, X_test_masks, X_test_eeg, X_test_gaze], y_test,
                                verbose=0)
        predictions = saved_model.predict([X_test_text, X_test_eeg, X_test_gaze] if embedding_type is not 'bert' else [X_test_text, X_test_masks, X_test_eeg, X_test_gaze])
