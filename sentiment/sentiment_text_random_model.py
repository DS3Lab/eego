...
def create_lstm_cognitive_model(param_dict, X_train_eeg_shape): # X_train_eeg_shape = (X_train_eeg.shape[1], X_train_eeg.shape[2])
    lstm_dim = param_dict['lstm_dim']
    dense_dim = param_dict['dense_dim']
    dropout = param_dict['dropout']
...
def create_inception_cognitive_model(param_dict, X_train_eeg_shape):  # X_train_eeg_shape = (X_train_eeg.shape[1], X_train_eeg.shape[2])
    ...
    input_eeg = Input(shape=X_train_eeg_shape, name='eeg_input_tensor') # eeg_input_tensor / gaze_input_tensor
...
def classifier(features, labels, eeg, embedding_type, param_dict, random_seed_value):
    ...
    # prepare EEG data TODO is this needed?
