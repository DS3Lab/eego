...
from tensorflow.python.keras.layers import Input, Dense, LSTM, Bidirectional, Flatten, Dropout, Conv1D, MaxPooling1D, GlobalMaxPooling1D, BatchNormalization, ReLU, AveragePooling1D, GlobalAveragePooling1D
...
def classifier(labels, eeg, embedding_type, param_dict, random_seed_value):
    ...
    for train_index, test_index in kf.split(X_data_eeg):
        ...
        if config.model is not 'cnn':
            print('WARNING: Model not set to cnn ... not that this would change anything')


        conv_1 = Conv1D(filters=inception_filters, kernel_size=inception_kernel_sizes[0], activation='elu', strides=1, use_bias=False, padding='same')(input_text)

        conv_3 = Conv1D(filters=inception_filters, kernel_size=inception_kernel_sizes[0], activation='elu', strides=1, use_bias=False, padding='same')(input_text)
        ...
        conv_5 = Conv1D(filters=inception_filters, kernel_size=inception_kernel_sizes[0], activation='elu', strides=1, use_bias=False, padding='same')(input_text)
        ...
        pool_proj = MaxPooling1D(pool_size=inception_pool_size, strides=1, padding='same')(input_text)
        ...
        # plotting the model
        # plot_model(model, show_shapes=True, show_layer_names=True, to_file='inception_model_4.png'.format(config.model))
