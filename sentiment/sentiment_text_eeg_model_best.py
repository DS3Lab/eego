...
def classifier(features, labels, eeg, embedding_type, param_dict_text, param_dict_eeg, random_seed_value):
    ...
    for train_index, test_index in kf.split(X_data_text):
        ...
        epochs = param_dict_text['epochs']
        lr = param_dict_text['lr']

        fold_results['params'] = [lstm_dim, lstm_layers, dense_dim, dropout, batch_size, epochs, lr, embedding_type, param_dict_eeg['lstm_dim'], param_dict_eeg['dense_dim'], param_dict_eeg['dropout'],
                                  random_seed_value]
